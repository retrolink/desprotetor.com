...
import urllib, urllib2, base64, urlparse, cgi, re, cgitb, hashlib, binascii, logging, simplejson
import settings
...
'''
db = MySQLdb.connect(host=settings.dbhost,user=settings.dbuser,passwd=settings.dbpass,db=settings.dbbase)
cursor = db.cursor()
'''
...
class UrlDeprotect:
    ...
    def insertDbSuccess(self):
        '''
        hash = self.parsed_url[1] + self.decode_method
        m = hashlib.md5()
        m.update(hash)
        hash = m.hexdigest()
        '''
        ...
        data["decoded_url"] = self.decodedUrl
        data["domain"] = self.parsedUrl[1]
        data["method"] = self.decodeMethod
        data["extension"] = self.extension
        logger_dbdata.info(simplejson.dumps(data))
        ''' 
        query = """INSERT INTO deprotect_py_success_data (hash,domain,method,num) VALUES ('""" + hash + """','""" + self.parsed_url[1] + """','""" + self.decode_method + """',1) ON DUPLICATE KEY UPDATE num = num +  VALUES(num);"""
        db.query(query)
        query = """INSERT INTO deprotect_py_success_urls (url,result,date) VALUES ('""" + self.originalurl + """','""" + self.decoded_url + """',DATE_ADD(NOW(),INTERVAL 6 HOUR));"""
        db.query(query)        
        r = db.store_result()
        '''
    ...
    def insertDbFailure(self):
        if self.originalurl.startswith("http"):
            ...
            logger_dbdata.info(simplejson.dumps(data))
            '''
            query = """INSERT INTO deprotect_py_fail (domain,url) VALUES ('"""+ self.parsed_url[1] +"""', '""" + self.originalurl + """');"""
            db.query(query)        
            r = db.store_result()
            '''

    def getDbAnswer(self):
        domain = self.parsedUrl[1]
        data = ""
        ...
        '''
            query = """select domain,answer,log_as_fail from deprotect_py_answer where domain = '"""+ self.parsed_url[1] + """'"""
