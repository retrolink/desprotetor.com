...
for s in list:   
    ...
    if not s == '':
        ...
        x.invalidUrls = ["http://www.link-protegido.com","http://www.desprotetordelink.com","http://track.ozonion.com/"]
        ...
        if not x.decoded:
            ...
            "zorrofilmes.com":"protcaHR0cDovL/",
        ...
        if not x.decoded:
            if x.parsedUrl[1] == "www.link-protegido.com":
                ...
                if not x.decoded:
                    ...
                    auth = soup.find('input', id="auth")['value']               


                    downurl = "http://www.desprotetordelink.com/fnc.php?op=4&url="+x.url.replace("&","@321")+"&auth="+auth
                    data = ''
...
#decidir o que fazer quando passou por todo o algoritmo
        if x.decoded:
            ...
            try:
                ...
                logger.error("Erro ao inserir sucesso: "+str(e))
            ...
            try:
                ...
                response = False
                logger.error("Erro ao pegar respostas padrao: "+str(e))

            if response:
                success = True

                outputlist.append({"dep":2,"url":x.url,"mess":response[1]})
